...
| 创建任务       | `create_feishu_task`             | 创建飞书任务           | 任务管理和跟踪                                 | ✅ 已完成 |
| **日历管理**   | `create_feishu_calendar_event`   | 创建日历事件           | 日程 CRUD、会议室预约、忙闲查询               | ✅ 已完成 |
| **消息管理**   | `get_feishu_message_content`     | 获取飞书消息内容       | 读取消息元数据、提取文件Key                   | ✅ 已完成 |
|                | `get_feishu_message_resource`    | 下载消息中的资源文件   | 下载消息附件（图片/文件），按原始文件名保存    | ✅ 已完成 |
...
- ~~**支持飞书消息文件下载**：通过消息ID获取消息内容及附件，支持按原始文件名（含扩展名）将文件/图片保存到指定目录~~ 0.2.1 ✅
---
